...
const QCommandLineOption SSHAgentPopulate::WaitOption = QCommandLineOption(QStringList() << "w"
                                                                                << "wait",
                                                                  QObject::tr("Wait for exit signal."));
...
SSHAgentPopulate::SSHAgentPopulate()
{
    ...
    optionalArguments.append({QString("entry"),
        ...
        QString("[entry]")});
    options.append(SSHAgentPopulate::WaitOption);
...
int SSHAgentPopulate::executeWithDatabase(QSharedPointer<Database> database, QSharedPointer<QCommandLineParser> parser)
{
    ...
    if (!parser->isSet(SSHAgentPopulate::WaitOption)) {
        out << QObject::tr("Key(s) added to the SSH agent\n");
        return EXIT_SUCCESS;
    }
